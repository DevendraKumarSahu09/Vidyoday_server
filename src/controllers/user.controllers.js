...
export const saveEduUnRegisteredUser = asyncHandler(async (req, res) => {
  ...
  education.forEach((edu) => {
    ...
    if (!edu.startDate || !edu.endDate || !edu.score || edu.score < 0 || edu.score > 100 || edu.startDate > edu.endDate) {
      throw new ApiError(400, "Please provide valid score and dates");
    }
  });
...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  const { name, email, username, linkedinLink, githubLink, portfolioLink, skillsProficientAt, skillsToLearn, education, bio, projects } = req.body;
  ...
  if (!unregisteredUser) {
    throw new ApiError(404, "No unregistered user found with the given email");
  }

  const avatarURL = req.file ? await uploadOnCloudinary(req.file) : unregisteredUser.avatarURL;
  ...
  const token = generateJWTToken_username(savedUser.username);

  sendMail({
    to: savedUser.email,
    subject: "Welcome to SkillShor",
    text: `Hello ${savedUser.name}, welcome to the SkillShor community!`,
  });

  return res
    .status(201)
    .json(new ApiResponse(201, { user: savedUser, token }, "User registered successfully"));
});
